...
public class CreateStoryActivity extends StoryActivityBase {
	...
	private void launchSoundIntent() {
		...
		AudioIntent.putExtra(SoundRecordActivity.EXTRA_OUTPUT,audioFile.getPath());
	...
	private void launchCameraIntent() {
		...
		//cameraIntent.setAction(INTENT_ACTION_STILL_IMAGE_CAMERA) ;
		...
		Uri imagePath = getOutputMediaFileUri(MEDIA_TYPE_IMAGE);
		fragment.imagePath = imagePath;
		Log.d(LOG_TAG, "path() :" + fragment.imagePath.toString());
		// TODO - Add the filename to the Intent as an extra. Use the Intent-extra name
	...
	private void launchVideoCameraIntent() {
		...
		Uri videoPath = Uri.fromFile(videoFile);

		
		// TODO - Add the filename to the Intent as an extra. Use the Intent-extra name
		// from the MediaStore class, EXTRA_OUTPUT
		videoIntent.putExtra(MediaStore.EXTRA_OUTPUT,videoFile.getPath());
